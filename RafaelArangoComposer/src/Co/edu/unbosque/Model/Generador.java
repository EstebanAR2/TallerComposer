...
public class Generador {
	...
	public String generarCancion() {
		...
		for (int i = 0; i < numEstrofas; i++) {
			...
			for (int j = 0; j < numFrasesPorEstrofa; j++) {
				cancion += generarFraseAleatoria() + " "; // Espacio entre palabras
			}
			cancion += "\n"; // Salto de línea después de cada estrofa
		}
	...
	public String generarFraseAleatoria() {
		int numPalabras = ThreadLocalRandom.current().nextInt(3, 9); // Escoge de 3 a 8 palabras aleatoriamente
		String frase = "";

		for (int i = 0; i < numPalabras; i++) {
			int randomIndex = ThreadLocalRandom.current().nextInt(0, palabras.length);
			frase += palabras[randomIndex] + " "; // Agrega palabras con un espacio en blanco entre ellas
		}
